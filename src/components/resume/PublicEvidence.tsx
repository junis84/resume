      ...
      <div className="public-evidence-grid grid grid-cols-2 gap-6">
          ...
          <div className="divide-y divide-stone-100">
            {publications.map((item) => (
              <div key={item.id} className="py-1.5 first:pt-0">
                <a href={item.url} className="t-detail font-semibold text-navy-900 hover:text-navy-700">
                  {item.title}
                </a>
        ...
        <div>
          <div className="flex items-baseline justify-between gap-3 mb-1">
            ...
            {githubUrl && <a href={githubUrl} className="t-meta text-navy-700">GitHub →</a>}
          </div>
          <div className="divide-y divide-stone-100">
            {openSource.map((item) => (
              <div key={item.id} className="flex items-baseline gap-2 py-1.5 first:pt-0">
                <a href={item.url} className="t-detail shrink-0 font-semibold text-navy-900 hover:text-navy-700">{item.name}</a>
                <p className="t-meta min-w-0 text-stone-600">{item.description}</p>
                <span className="t-meta ml-auto shrink-0 text-stone-500">{item.language}</span>
              </div>
            ))}
          </div>
        </div>
